...
modelSelect.addEventListener('change', () => {
    ...
    onValue(modelRef, (snapshot) => {
        ...
        if (modelData) {
            phoneImage.src = `${IMG_BASE}${selectedModel}.png`;
            phoneImage.alt = selectedModel;
            phoneImage.onerror = () => { phoneImage.src = defaultImageSrc; };
            ...
            repairs.forEach((repair) => {
                ...
                cb.type = 'checkbox';
                cb.dataset.price = modelData[repair];
                cb.addEventListener('change', updateTotal);
                let text = repair + ': $' + modelData[repair];
                text = text.replace(/screen/gi, '<span style="color: lightsteelblue;">Screen</span>');
                text = text.replace(/battery/gi, '<span style="color: #5aaa6a;">Battery</span>');
...
function updateTotal() {
    let total = 0;
    repairOptions.querySelectorAll('input[type="checkbox"]:checked').forEach(cb => {
...
document.getElementById('Apple').addEventListener('change', () => { loadData(); localStorage.setItem("selectedPhoneBrand", "Apple"); });
document.getElementById('Samsung').addEventListener('change', () => { loadData(); localStorage.setItem("selectedPhoneBrand", "Samsung"); });
document.getElementById('Other').addEventListener('change', () => { loadData(); localStorage.setItem("selectedPhoneBrand", "Other"); });

const storedBrand = localStorage.getItem("selectedPhoneBrand");
if (storedBrand === "Apple") document.getElementById('Apple').checked = true;
else if (storedBrand === "Samsung") document.getElementById('Samsung').checked = true;
else if (storedBrand === "Other") document.getElementById('Other').checked = true;
